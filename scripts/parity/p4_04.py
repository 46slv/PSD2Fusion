...
def _graph(comp_path: Path) -> Dict[str, Any]:
    ...
    local_controls: List[Tuple[str | None, str | None]] = [
        (tool["apply_mode"], tool["blend"]) for tool in stacks
    ]
    return {
        "tools": tools,
...
def build(output: Path) -> Dict[str, Any]:
    ...
    expected_primary = ('FuID { "Multiply" }', "0.500000")
    expected_alternate = ('FuID { "Screen" }', "0.250000")
    expected_members = [
        ('FuID { "%s" }' % FUSION_BLEND_IDS[mode], "%.6f" % opacity)
    ...
    checks = {
        ...
        "primary_outer_owns_base_controls": (
            outer is not None
            and (outer["apply_mode"], outer["blend"]) == expected_primary
        ),
        "alternate_outer_tracks_changed_base_controls": (
            alternate_outer is not None
            and (alternate_outer["apply_mode"], alternate_outer["blend"])
            == expected_alternate
        ),
        ...
        "outer_boundary_is_after_complete_local_stack": (
            outer is not None
            and outer["foreground"] == primary["stacks"][-1]["name"]
            and outer["start"] > max(tool["start"] for tool in primary["stacks"])
        ),
        "outer_boundary_is_explicit": (
            outer is not None and "P4-04 base blend/opacity once" in outer["comments"]
        ),
    }
    report: Dict[str, Any] = {
        "schema_version": 1,
