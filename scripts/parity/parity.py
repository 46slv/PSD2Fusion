...
def run_inspection(psd_path: str, reference_path: str, output: Path) -> Dict[str, Any]:
    before_psd, before_ref = inspect_psd(psd_path), inspect_input(reference_path, "reference"); norm = _qualification(before_psd, before_ref); valid = bool(before_psd.get("exists") and not before_psd.get("error") and before_ref.get("exists") and not before_ref.get("error")); result: Dict[str, Any] = {"schema_version": 2, "status": "PASS" if valid and norm.get("classification") == "exact_canvas" else "BLOCKED", "psd": before_psd, "reference": before_ref, "normalization": norm, "environment": _environment(), "command": " ".join(sys.argv)}
    if valid:
        try:
            from psd_tools import PSDImage
    ...
    result["after_sha256"] = {"psd": _sha256(Path(psd_path)) if Path(psd_path).is_file() else None, "reference": _sha256(Path(reference_path)) if Path(reference_path).is_file() else None}; result["inputs_unchanged"] = bool(valid and result["after_sha256"].get("psd") == before_psd.get("sha256_before") and result["after_sha256"].get("reference") == before_ref.get("sha256_before")); result["exit_codes"] = {"inspect": 0 if result.get("status") == "PASS" and result.get("inputs_unchanged") else 3, "stored_composite_compare": (0 if result.get("stored_composite_comparison", {}).get("status") == "PASS" else 2) if "stored_composite_comparison" in result else None}; result["privacy"] = {"private_input_paths_committed": False, "safe_summary": True}; output.parent.mkdir(parents=True, exist_ok=True); output.write_text(json.dumps(_safe_summary(result), indent=2, ensure_ascii=False), encoding="utf-8"); return result
