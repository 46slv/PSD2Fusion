...
def compare_images(candidate: str | os.PathLike[str] | Image.Image, reference: str | os.PathLike[str] | Image.Image,
    ...
    rgba_indices = [i for i, d in enumerate(diffs) if any(d)]
    rgb_indices = [i for i, d in enumerate(diffs) if any(d[:3])]
    alpha_indices = [i for i, d in enumerate(diffs) if d[3]]
    changed = [(i % c.width, i // c.width) for i in rgba_indices]; bbox = None
